...
- **External Cluster Rather**: Rather than using kind / minikube for reproduce the deployment testing, I have used my HomeLabs cluster for deploying the test application and expose the endpoint via Envoy Loadbalancer that is already configured for Production use case remediation. In a real-world scenario, you would set up kind or minikube, install an Ingress Controller (like ingress-nginx or Traefik), and configure the Ingress resource accordingly. This Ingress resource would route external HTTP/HTTPS traffic to the `ledger-api` service. The specific port (like 8000 in this case) would be mapped to the service's targetPort.
